...
class EPUBParser:
    ...
    async def extract_metadata(self, epub_path: str) -> BookCreate:
        ...
        try:
            ...
            raise EbookParsingError(
                f"Failed to parse EPUB: {str(e)}", {"path": epub_path, "error": str(e)}
            ) from e
    ...
    async def extract_cover(self, epub_path: str) -> str | None:
        ...
        try:
            ...
            epub_hash = hash_path(epub_path)
            cover_path = self.covers_path / f"{epub_hash}.jpg"

            # Handle SVG covers
            if is_svg or (
                hasattr(cover_item, "media_type") and cover_item.media_type == "image/svg+xml"
            ):
    ...
    async def get_chapters(self, epub_path: str) -> list[tuple[int, str, str]]:
        ...
        try:
            ...
            raise EbookParsingError(
                f"Failed to extract chapters: {str(e)}", {"path": epub_path}
            ) from e

    async def get_single_chapter(self, epub_path: str, chapter_index: int) -> tuple[str, str, int]:
        ...
        try:
            ...
            raise EbookParsingError(
                f"Failed to extract chapter: {str(e)}", {"path": epub_path, "index": chapter_index}
            ) from e
    ...
    def _filter_spine_items(self, book: "epub.EpubBook") -> list[tuple]:
                ...
                if image_item is None:
                    ...
                    for book_item in book.get_items():
                        if book_item.get_type() == ebooklib.ITEM_IMAGE:
                            ...
                            if (
                                item_name == src
                                or item_name.endswith(src)
                                or item_name.endswith(src_normalized)
                            ):
    ...
    async def get_table_of_contents(self, epub_path: str) -> list[dict]:
        ...
        try:
            ...
            def resolve_href(toc_item: Any) -> int | None:
            ...
            def process_toc_item(item: Any, level: int = 1) -> None:
                ...
                if isinstance(item, (tuple, list)):
                    ...
                    if spine_idx is not None:
                        toc_items.append(
                            {"index": spine_idx, "title": item.title or "Untitled", "level": level}
                        )
