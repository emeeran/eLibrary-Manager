...
def _add_book(
    ...
    *,
    ...
    if comment:
        conn.execute(
            "INSERT INTO comments (id, book, text) VALUES (?, ?, ?)", (book_id, book_id, comment)
        )
    if tags:
...
async def test_calibre_status_rejects_bad_path(client, tmp_path: Path) -> None:
    ...
    resp = await client.get("/api/library/calibre-status", params={"path": str(tmp_path)})
    assert resp.status_code == 200
    body = resp.json()
...
async def test_import_library_incremental_update(calibre_library: dict, tmp_path: Path) -> None:
