...
(function () {
  ...
  class ServerTTS {
    ...
    async speakSegments(segments, options = {}) {
      ...
      try {
        ...
        let prefetchPromise =
          segments.length > 1
            ? _fetchAudioChunk(segments[1], voice, rate, pitch).catch(
                () => null,
              )
            : null;
        ...
        for (let i = 1; i < segments.length; i++) {
          ...
          const blob =
            ...
            (await _fetchAudioChunk(segments[i], voice, rate, pitch).catch(
              () => null,
            ));
          if (!blob) break;
          ...
          prefetchPromise =
            i + 1 < segments.length
              ? _fetchAudioChunk(segments[i + 1], voice, rate, pitch).catch(
                  () => null,
                )
              : null;
  ...
  async function initTTS() {
    const engineSelect =
      ...
      document.getElementById("tts-service-select");
  ...
  async function updateVoiceList() {
    ...
    if (engine === ENGINE_EDGETTS) {
      ...
      voiceSelect.innerHTML = edgeTTSVoices
      ...
      voiceSelect.innerHTML = gttsVoices
        .map(
          (voice) =>
  ...
  function segmentSentencesAndWords() {
    ...
    if (!chapterText) {
      console.warn("[TTS] #ic-chapter-text not found");
      return;
    }

    // Unwrap any previous tts-sentence and tts-word spans
    chapterText
      .querySelectorAll("span.tts-sentence, span.tts-word")
      .forEach((span) => {
  ...
  function _startSegmentTracking(segIndex) {
    ...
    const rate =
      parseFloat(document.getElementById("speed-selector")?.value || 1.0) ||
      1.0;
  ...
  function clearHighlights() {
    ...
    chapterText
      .querySelectorAll(".tts-word-highlight, .tts-word-spoken")
      .forEach((el) => {
  ...
  window.tts = {
    ...
    getRate: () =>
      parseFloat(localStorage.getItem("dawnstar_tts_rate") || "1.0"),
    setVolume: (volume) => {
      if (audioElement) {
