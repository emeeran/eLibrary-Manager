...
async function openReaderInCalibreWeb() {
  ...
  try {
    const res = await fetch(
      `/api/books/${IcecreamReader.bookId}/calibre-web-url`,
    );
...
function printChapter() {
  const title =
    ...
    document.getElementById("ic-book-title")?.textContent ||
    "Chapter";
  let htmlContent = "";
  ...
  if (IcecreamReader.pageLayout === "continuous") {
    ...
    const dividers = contentArea
      ? contentArea.querySelectorAll(".ic-chapter-divider")
      : [];
    let startEl = null;
    let endEl = null;
    dividers.forEach((d) => {
