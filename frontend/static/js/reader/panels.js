...
function toggleTOCPanel() {
  ...
  if (
    IcecreamReader.activeLeftPanel === "toc" &&
    !sidebar.classList.contains("collapsed")
  ) {
    sidebar.classList.add("collapsed");
    document
      .getElementById("ic-tab-contents")
      ?.classList.remove("ic-tab-active");
  } else {
...
function closeBookmarksPanel() {
  ...
  document
    .getElementById("ic-tab-bookmarks")
    ?.classList.remove("ic-tab-active");
}
...
async function switchSummaryTab(tab) {
  ...
  if (tab === "chapter") {
    ...
    summaryContainer.innerHTML =
      '<div class="ic-loading" style="text-align: center; padding: 40px 20px;"><div class="ic-spinner"></div><p style="margin-top:12px;font-size:12px;color:#999;">Checking for cached summary...</p></div>';
    try {
      const resp = await fetch(
        `/api/books/${IcecreamReader.bookId}/summary/${IcecreamReader.currentChapter}`,
      );
      if (resp.ok) {
...
async function loadBookInfo() {
  try {
    ...
    const pct = book.progress
      ...
      : totalChapters > 0
        ? Math.round(
            ((IcecreamReader.currentChapter + 1) / totalChapters) * 100,
          )
        : 0;
    ...
    if (book.publish_date)
      details.push(
        `<strong>Published:</strong> ${escapeHtml(String(book.publish_date))}`,
      );
    if (book.language)
      details.push(`<strong>Language:</strong> ${escapeHtml(book.language)}`);
    if (book.isbn)
      details.push(`<strong>ISBN:</strong> ${escapeHtml(book.isbn)}`);
    if (book.file_size) {
    ...
    const starsHtml = [1, 2, 3, 4, 5]
      ...
      .join("");
    details.push(
      `<strong>Rating:</strong> <span class="ic-rating-stars">${starsHtml}</span>`,
    );

    // Categories
    if (book.categories && book.categories.length) {
      details.push(
        `<strong>Categories:</strong> ${book.categories.map((c) => escapeHtml(c)).join(", ")}`,
      );
    }
    ...
    if (descEl) {
      ...
      if (details.length)
        html +=
          '<div class="ic-bookinfo-meta">' + details.join("<br>") + "</div>";
      html += descText
