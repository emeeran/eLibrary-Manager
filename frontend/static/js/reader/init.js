...
function initEventListeners() {
  ...
  if (progressSlider) {
    ...
    progressSlider.addEventListener("input", function (e) {
      ...
      const targetChapter = Math.floor(
        (progress / 100) * IcecreamReader.totalChapters,
      );
      if (
        targetChapter !== IcecreamReader.currentChapter &&
        targetChapter < IcecreamReader.totalChapters
      ) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (autoSummaryEl) {
    autoSummaryEl.checked =
      localStorage.getItem("reader-auto-summary") === "true";
  }
});
...
document.addEventListener("keydown", function (e) {
  ...
  switch (e.key) {
    ...
    case "End":
      if (e.ctrlKey || e.metaKey) {
        e.preventDefault();
        if (IcecreamReader.totalChapters > 0)
          loadChapter(IcecreamReader.totalChapters - 1);
      }
      break;
    case "+":
    case "=":
