...
function renderAIProviders(providers) {
  ...
  if (!providers.length) {
    container.innerHTML =
      '<div class="ic-ai-provider-loading">No providers configured</div>';
    return;
  }
  ...
  container.innerHTML = providers
    .map((p) => {
      ...
      const badge = p.available
        ? p.is_current
          ? '<span class="ic-ai-provider-badge">Active</span>'
          : '<span class="ic-ai-provider-badge">Ready</span>'
...
async function testAIConnection() {
  ...
  if (provider === "ollama_local") {
    ...
    if (!url) {
      showToast("Enter Ollama URL");
      return;
    }
    btn.className = "ic-ai-test-btn testing";
    btn.innerHTML =
      '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 4V2A10 10 0 0 0 2 12h2a8 8 0 0 1 8-8z" opacity="0.3"><animateTransform attributeName="transform" type="rotate" from="0 12 12" to="360 12 12" dur="1s" repeatCount="indefinite"/></path></svg> Testing';
    try {
      ...
      if (resp.ok) {
        ...
        btn.innerHTML =
          '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg> OK';
      } else {
      ...
      btn.innerHTML =
        '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg> Fail';
    }
    setTimeout(() => {
      btn.className = "ic-ai-test-btn";
      btn.innerHTML =
        '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg> Test';
    }, 3000);
    return;
  }

  if (!apiKey) {
    showToast("Enter API key");
    return;
  }
...
async function saveAIKey() {
  ...
  const apiKey = document.getElementById("ic-ai-key-input")?.value?.trim();
  if (!provider || !apiKey) {
    showToast("Enter provider and API key");
    return;
  }
